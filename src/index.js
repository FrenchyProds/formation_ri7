import { slugify, fetchDB } from "./modules/utils.mjs"
...
window.onload = () => {
    ...
    searchBar.addEventListener("keyup", function(event) {
        ...
        if(event.key === "Enter" && sluggedText.length > 2 || sluggedText.length === 0) {
            searchButton.click()
        } else if (event.key === "Enter" && sluggedText.length <= 2) {
            card.innerHTML = 
            errorTemplate(
    ...
    searchButton.addEventListener('click', (e) => {
        ...
        fetchDB('../data/adresses.json').then(res => {
            let results = []
            let loading = true
            const sluggedText = slugify(searchContent)
            res.forEach(item => {
                const {label, city, context} = item
            ...
            if (results.length === 0 && searchContent) { 
